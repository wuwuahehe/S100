...
{
    int i;
    u8 *dblkey = &xcfg_cb.dblkey_num0;

    if (xcfg_cb.key_multi_config_en) {
        for (i = 0; i < 5; i++) {
            if (key_val == ((u16)key_config_table[dblkey[i]] | KEY_SHORT_UP)) {
                return 2;
            }
        }
        return 1;
...
const adkey_tbl_t adkey_table[] = {
    ...
    #if KARAOK_RM_VOICE
    {0xA8, KEY_VOICE_RM},       //VOICE RM      15K,
    #elif FUNC_REC_EN
...
u8 *get_adkey_configure(u8 num)
{
    if (xcfg_cb.adkey_config_en) {
        ...
        if(num>=3){
            sys_cb.aux_sd_detect_flag = 0;
            sys_cb.aux_sd_flag = 0;
            sys_cb.aux_sd_flag2 = 0;
        }else
            if(num==1){  //SD卡在线
                sys_cb.aux_sd_detect_flag = 1;
                sys_cb.aux_sd_flag2 = 1;
            }else if(num==2){  //AUX在线
                sys_cb.aux_sd_detect_flag = 2;
                sys_cb.aux_sd_flag2 = 2;
            }else if(num == 0){
                sys_cb.aux_sd_detect_flag = 3;  //AD和AUX都在线
            }
