...
extern u8 freq_disp_en;
#endif

#if BT_TWS_EN  && BT_TWS_SLAVE_ID3
u8 id3_buf[ID3_BUF_LEN];
void tws_send_id3_info(void);
uint16_t tws_get_id3_info(uint8_t lid, uint8_t *buf)
{
    if(buf != NULL) {
        memcpy(buf, id3_buf, id3_len);
    }
    return id3_len;
}

void tws_set_id3_info(uint8_t *buf, uint16_t len)
{
    bt_id3_tag_callback(0,buf,len);
}
#else
u8 id3_buf[ID3_BUF_LEN]; //AT(.wavres.buf);  //ID3 BUF跟TWS音乐BUF RAM复用
#endif
        ...
        switch (index) {
            case BT_ID3_TITLE:
                ...
                if(!freq_disp_en) {
                    ...
                    buf[offset-4] = 'D';
                    buf[offset-3] = '3';
                    buf[offset-2] = length>>8;
                    buf[offset-1] = length&0xFF;
                    huart_putbuf(&buf[offset-5], length+5);
                }
#endif
                break;
