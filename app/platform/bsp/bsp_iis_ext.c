...
u8 iis_io_init(TYPE_IIS_IO io_map, TYPE_IIS_MODE iis_mode, TYPE_MCLK_OUT_SEL mclk_out_en)
{
    ...
    if (IIS_G1 == io_map) {
        ...
        if (iis_mode & IISCFG_MASTER) {  //MASTER,BCLK,LRC,MCLK out
            ...
            GPIOADIR |= BIT(3) | BIT(4)| BIT(5);
            GPIOAPU |= BIT(3) | BIT(4)| BIT(5);
        }
        ...
        if (iis_mode & IISCFG_MASTER) {  //MASTER,BCLK,LRC,MCLK out
            ...
            TRACE("SLAVE ");
            GPIOEFEN |= BIT(0) | BIT(6)| BIT(7);;
            GPIOEDE |= BIT(0) | BIT(6)| BIT(7);;
            GPIOEDIR |= (BIT(0) | BIT(6))| BIT(7);;
            GPIOEPU |= BIT(0) | BIT(6)| BIT(7);
        }
        if (iis_mode & IISCFG_TX) {  //DO out
            TRACE("TX_DO_OUT ");
            GPIOEFEN |= BIT(5);
...
void aubuf_adjust(void)
{
    ...
    u16 aubuf_fifo_cnt = AUBUFFIFOCNT & 0xffff;
    if(CKB0(DACDIGCON0,6)) {  //phasecomp sync enable
        SETB(DACDIGCON0,6);
    }
    if (aubuf_fifo_cnt <= (au_size * 2)) {        //2 / 16 -> 0xFF80
        PHASECOMP = 0xF80;     //PHASECOMP [11:0] valid
    } else if (aubuf_fifo_cnt <= (au_size * 4)) { //4 / 16 -> 0xFFF0
        PHASECOMP = 0xFF0;
    } else if (aubuf_fifo_cnt <= (au_size * 6)) { //6 / 16 -> 0xFFF8
        PHASECOMP = 0xFFE;
    } else if (aubuf_fifo_cnt >= (au_size*14)) {  //14 / 16 -> 0x0010
        PHASECOMP = 0x080;
    } else if (aubuf_fifo_cnt >= (au_size*12)) {  //12 / 16 -> 0x0004
        PHASECOMP = 0x020;
    } else if (aubuf_fifo_cnt >= (au_size*10)) {  //10 / 16 -> 0x0002
        PHASECOMP = 0x010;
    } else {
        PHASECOMP = 0;
    }
    ...
    if (iis_cfg.mode & IISCFG_SRC) {  //有SRC输出
        ...
        while(1) {
            ...
            if((AUBUFCON & BIT(8)) == 0) {  //AUBUFCON BIT(8)为1时表示DACBUF已满,为0表示DACBUF未满,可以往里面写数据
                AUBUFDATA = get_tbl_pcm((void*)iis_dmabuf,sizeof(iis_dmabuf));  //通过AUBUFDATA向DACBUF写数据,DACBUF内部大约有2K的缓存
                samples++;
            }
        }
    }
