...
void udisk_dpdm_release(bool release);

#if FUNC_MUSIC_EN

u32 mp3_offset_beginning = 512*0;  //开头偏移多少byte
u32 mp3_offset_ending    = 0;      //结尾偏移多少byte
bool mp3_loopback_en(void)
{
    return MUSIC_MP3_LOOPBACK_EN;
}
void mp3_loopback_start(void)      //重新开始循环
{
...
#if UDE_STOAGE_FLASH_EN
    if(spiflash1_manu_id_read()){
        if(sys_cb.cur_dev != DEV_SPIFLASH){
...
bool func_music_auto_next_device(void)
{
    ...
    if(f_msc.cur_dev == DEV_UDISK){
        ...
        udisk_suspend();
        #if SD_USB_MUX_IO_EN
        udisk_dpdm_release(1);   //关USB SIE功能,避免影响SD复用USB口
        #endif
    }if(sys_cb.cur_dev == DEV_UDISK){
        printf("USB Mode.\n");
        udisk_resume();      //内部相当于调用udisk_dpdm_release(0),恢复USB SIE功能
    }
    led_music_busy();
        ...
        if (fs_open_num(f_msc.file_num)) {
            ...
            if(f_msc.type != NONE_TYPE) {
                ...
                if (f_msc.type == MP3_TYPE) {
                    music_mp3_loopback_size(0x10,0x10);    //首尾偏移
                }
#endif
            }
        }
...
{
    ab_bp_var.b_sta_play_time = 0;
    ab_bp_var.cur_file_num = 0;
    ab_bp_var.ab_sta = AB_STA_NON;
}
...
void func_music_ab_breakpoint_switch(void)
{
    ...
    switch(ab_bp_var.ab_sta){
        case AB_STA_NON:
            music_get_breakpiont(&ab_bp_var.a_sta_bp);
            ab_bp_var.cur_file_num = f_msc.file_num;
            ab_bp_var.ab_sta = AB_STA_A;
            printf("AB_STA_A\n");
            break;
        case AB_STA_A:
            ab_bp_var.b_sta_play_time = music_get_cur_time();
            ab_bp_var.ab_sta = AB_STA_B;
            printf("AB_STA_B\n");
            break;
        case AB_STA_B:
            func_music_ab_breakpoint_clear();
...
#if UDE_STOAGE_FLASH_EN
        if(spiflash1_manu_id_read() && (f_msc.cur_dev == DEV_SPIFLASH)){
...
//是否过滤空的文件，默认false
//AT(.text.fs.scan)
//bool empty_files_filter_en(void)
//{
//    return false;
//}
...
static void func_music_enter(void)
{
    ...
    if (is_sd_insert_reset()) {
        dev_sta_online_set(DEV_SDCARD);
        sd_insert_reset_clear();
        printf("sd_reset_clear\n");
    }
#endif
