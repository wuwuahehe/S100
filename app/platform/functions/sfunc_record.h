...
//record压缩数据BUF管理
typedef struct {
    u16 size;                       //record buf size
volatile u16 len;                   //record buf valid data bytes
    u8 *buf;                        //record buf start address
    u8 *wptr;                       //record buf write point
    u8 *rptr;                       //record buf read point
} au_stm_t;
...
bool puts_stm_buf(au_stm_t *stm, u8 *buf, u16 len);
bool gets_stm_buf(au_stm_t *stm, u8 *buf, u16 len);
